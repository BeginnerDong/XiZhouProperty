<template>
	<view>
		...
		<view class="editLine mx-3">
			...
			<view class="item  d-flex a-center j-sb">
				<view class="ll">问题地点</view>
				<view class="rr d-flex a-center j-end">
					<view class="d-flex a-center">
						<view class=" color9">
							<picker mode="selector" @change="choosePlace" :range="placeData" range-key="name">
								{{placeData[index1]?placeData[index1].name:'请选择'}}
							</picker>
						</view>
						<image class="arrowR" src="../../static/images/icon.png" mode=""></image>
					</view>
			...
			<view class="item  d-flex a-center j-sb">
				<view class="ll">巡检结果</view>
				<view class="rr d-flex a-center j-end">
					<view class="d-flex a-center mgr25" @click="setChange('2')">
						<image class="setIcon" :src="submitData.behavior==2?'../../static/images/probleml-icon3.png':'../../static/images/probleml-icon2.png'" mode=""></image>
						<view>正常</view>
					</view>
					<view class="d-flex a-center ml-5" @click="setChange('1')">
						<image class="setIcon" :src="submitData.behavior==1?'../../static/images/probleml-icon3.png':'../../static/images/probleml-icon2.png'" mode=""></image>
						<view>有问题</view>
					</view>
				</view>
			</view>
	...
	<!-- 	<view class=" typeShow rounded10 bg-white font-26 px-3" v-show="is_typeShow">
		...
		<!-- 问题地点 -->
		<!-- <view class=" typeShow rounded10 bg-white font-26 px-3" v-show="is_addrsShow">
			<view class="item d-flex a-center j-sb" v-for="(item,index) in addrsArray" :key="index" @click="addrsChange(index)">
				<view>{{item}}</view>
				<view class="icon"><image :src="addrsCurr==index?'../../static/images/probleml-icon1.png':''" mode=""></image></view>
			</view>
		</view> -->
...
<script>
	export default {
		...
		onLoad(options) {
			...
			self.$Utils.loadAll(['getLabelData','getUserInfoData','getPlaceData'], self);
		},
		...
		methods: {
			...
			choosePlace(e){
				...
				self.submitData.relation_id = self.placeData[self.index1].id;
			},
			...
			setChange(seltCurr){
				...
				if(seltCurr!=self.submitData.nameless){
					self.submitData.behavior = seltCurr;
				}
			},
			...
			getPlaceData() {
				...
				const callback = (res) => {
					if (res.info.data.length > 0) {
						self.placeData = res.info.data;
					};
					self.$Utils.finishFunc('getPlaceData');
				};
				self.$apis.placeGet(postData, callback);
			},
